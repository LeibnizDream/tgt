...
class TranscriptionStrategyFactory:
    @staticmethod
    def get_strategy(language_code: str, device: str) -> TranscriptionStrategy:
        if language_code in ['en', 'fr', 'de', 'es', 'it', 'ja', 'zh', 'nl',
                             'uk', 'pt', 'ar', 'cs', 'ru', 'pl', 'hu', 'fi',
                             'fa', 'el', 'tr', 'da', 'he', 'ko', 'ur', 
                             'te', 'hi', 'ca', 'ml', 'ro', 'ka', 'tl']:
            return WhisperxStrategy(language_code, device=device)
        elif language_code in ['th']:
            return ThaiTranscriptionStrategy(language_code, device=device)
        elif language_code in ['vi']:
            return VietnameseStrategy(language_code, device=device)
        elif language_code in ['et']:
            return WhisperStrategy(language_code, device=device)
        elif language_code in ['bn']:
            return BengaliStrategy(language_code, device=device)
